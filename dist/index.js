...
const main = async () => {
    const multiProvider = await getMultiProvider(chainMetadata).catch((error) => (log.error(error.reason ?? error.message), process.exit(1)));
    ...
    try {
        ...
        log.error({ msg: "Failed to initialize solvers", error });
        process.exit(1);
    }
};
await main();
